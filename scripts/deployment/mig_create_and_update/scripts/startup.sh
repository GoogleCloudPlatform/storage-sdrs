...
sleep 5
docker run --detach -v /root/cred:/var/sdrs --name=sdrs --env-file=./env.txt --log-driver=gcplogs --publish=8080:8080 --net=esp_network gcr.io/sdrs-server/eshen-sdrs-demo:private-db-1 # gcr.io/YOUR_GOOGLE_PROJECT_ID/YOUR_CONTAINER_IMAGE:TAG. To be updated when updating the MIG.
rm -rf ./env.txt
sleep 30
