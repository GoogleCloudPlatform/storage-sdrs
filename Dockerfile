...
CMD if [ "$ENABLE_JMX" = true ]; \
    then java -Xms256m -Xmx512m -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=8086 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -jar ./app.jar 0.0.0.0; \
    else java -jar ./app.jar 0.0.0.0 -Xms256m -Xmx512m; \
    fi
